...
public class OrderController {
	...
	public ModelAndView orderForm(HttpSession session, CartDTO cDTO) {
		...
		if (cDTO.getBundle() != null && cDTO.getBundle().length() > 0
				&& !cDTO.getBundle().contains("단품")) {
		}
	...
	public String oneGoodsOrder(HttpSession session, OrderinfoDTO oiDTO) {
		...
		if (oiDTO.getBundle() != null && oiDTO.getBundle().length() > 0
				&& !(oiDTO.getBundle().contains("단품"))) {
		}
		...
		if (goodsAmount - (oiDTO.getGamount() * bundle) < 0) {	// 재고보다 많은 수량의 상품 구매 시
			...
			if (oiDTO.getBundle() == null || oiDTO.getBundle().length() < 1) {
				// 소매 상품일 경우
			}
